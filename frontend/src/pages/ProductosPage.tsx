...
export default function ProductosPage() {
  ...
  const [initialForm, setInitialForm] = useState<ProductoForm | null>(null);
  ...
  const openEdit = (p: Producto) => {
    setEditing(p);
    const initialData = { codigo: p.codigo, nombre: p.nombre, descripcion: p.descripcion || '', precio: p.precio, stock: p.stock, activo: p.activo };
    setForm(initialData);
  ...
  const requestCloseModal = () => {
    ...
    const hasChanges = (Object.keys(initialForm) as (keyof ProductoForm)[]).some(
