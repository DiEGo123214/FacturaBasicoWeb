...
import { facturasApi, clientesApi, productosApi, MetodosPago, type Factura, type ClienteForm } from '../api';
import {
  UserIcon,
  ShoppingCartIcon,
  TrashIcon,
...
export default function FacturacionPage() {
  ...
  useEffect(() => {
    if (cart.loadedFromDuplicate) {
      toast.success('Datos de la factura cargados para una nueva venta.', { id: 'dup-loaded' });
      cart.setLoadedFromDuplicate(false);
    }
  }, [cart.loadedFromDuplicate, cart.setLoadedFromDuplicate]);
  ...
  const handleFinalizar = async () => {
    ...
    try {
      ...
      try {
        await clientesApi.getById(cart.clienteId);
      } catch {
        toast.error('El cliente seleccionado ya no existe. Selecciona otro cliente.');
        cart.setCliente(null, ''); // ← solo limpia el cliente, carrito queda intacto
        setLoading(false);
        return;
      }

      // ✅ Verificar que todos los productos aún existen y están activos
      const eliminados: number[] = [];
      await Promise.all(
        cart.items.map(async (item) => {
          try {
            await productosApi.getById(item.productoId);
          } catch {
            eliminados.push(item.productoId);
          }
        })
      );

      if (eliminados.length > 0) {
        const nombres = cart.items
          .filter(i => eliminados.includes(i.productoId))
          .map(i => i.nombre)
          .join(', ');
        toast.error(`Productos no disponibles: ${nombres}. Retíralos del carrito para continuar.`);
        setInvalidProductIds(new Set(eliminados)); // ← marca en rojo, NO limpia el carrito
        setLoading(false);
        return;
      }

      // Limpiar marcas rojas si todo está ok
  ...
  const requestCloseNuevoClienteModal = () => {
    const hasChanges = (Object.keys(emptyNuevoClienteForm) as (keyof ClienteForm)[]).some(
      key => nuevoClienteForm[key] !== emptyNuevoClienteForm[key]
    );
    if (hasChanges) {
  ...
  const nuevoClienteInputClass = (field: string) =>
    `w-full bg-zinc-50 border px-5 py-3.5 rounded-xl text-zinc-800 text-sm font-bold outline-none transition-all ${nuevoClienteTouched[field] && nuevoClienteErrors[field]
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="flex-1 overflow-auto custom-scrollbar">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-zinc-100">
                {cart.items.map(item => (
                  <tr key={item.productoId} className={`transition-colors group ${invalidProductIds.has(item.productoId) ? 'bg-red-100 hover:bg-red-200' : 'hover:bg-emerald-50/50'}`}>
                    ...
                    <td className="px-4 py-5 text-center">
                      <button onClick={() => {
                        if (invalidProductIds.has(item.productoId)) {
                          ...
                          setConfirmEliminarItem(item.productoId);
                        }
                      }} className="p-2 text-zinc-300 hover:text-red-600 transition-all"><TrashIcon className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-8 shadow-sm flex flex-col">
          ...
          <div className="mt-auto pt-6 border-t border-zinc-200 space-y-4">
            ...
            <button onClick={() => { if (cart.items.length > 0 || cart.clienteId) { setConfirmCancelarVenta(true); } else { cart.clearCart(); } }} className="w-full py-3.5 bg-transparent border-2 border-red-100 text-red-500 hover:bg-red-50 hover:text-red-600 hover:border-red-200 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all">
